...
Работающий в браузере инструмент для обработки и интерпретации данных
**георадиолокации (Ground-Penetrating Radar, GPR)**, ориентированный на
**скоростной анализ методом общей средней точки (Common Mid-Point, CMP)**.

Приложение создано в первую очередь для семейства георадаров **«Лоза»**.
Радары «Лоза» запускают приёмник по приходу прямой волны от передатчика,
поэтому записанные трассы привязаны к этому первому вступлению, а не к
фиксированному нулевому времени. Из-за этого исходные радарограммы выглядят
несколько нестандартно по сравнению с обычными данными георадиолокации,
и расчёт годографа CMP скорректирован с учётом этой особенности
(см. [Годограф с учётом специфики «Лозы»](#loza-specific-moveout) ниже).

> **Онлайн-демо:** https://teodorro.github.io/gpr-dowser/
> При запуске приложение загружает встроенный пример файла «Лоза», так что
> его можно попробовать без собственных данных.
